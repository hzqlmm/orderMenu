...
<form name="frm_main">
	...
	<div class="x_col2"><input type="text" name="DB_HOST" value="<?php echo fun_get::get('DB_HOST','localhost');?>" class="pTxt1 pTxtL200"> <font style="color:#ff0000">*</font></div>
	<div class="x_col3 pBeta">本机请填写localhost</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_col1">数据库名：</div>
	<div class="x_col2"><input type="text" name="DB_NAME" value="<?php echo fun_get::get('DB_NAME','kj_meal');?>" class="pTxt1 pTxtL200"> <font style="color:#ff0000">*</font></div>
	<div class="x_col3 pBeta">若不存在将尝试创建</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_col1">数据库用户名:</div>
	<div class="x_col2"><input type="text" name="DB_USER" value="<?php echo fun_get::get('DB_USER','root');?>" class="pTxt1 pTxtL200"> <font style="color:#ff0000">*</font></div>
	<div class="x_col3 pBeta">用于数据库连接验证的用户名</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_col1">数据库密码:</div>
	<div class="x_col2"><input type="password" name="DB_PWD" value="<?php echo fun_get::get('DB_PWD');?>" class="pTxt1 pTxtL200"> <font style="color:#ff0000">*</font></div>
	<div class="x_col3 pBeta">用于数据库连接验证的密码</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_col1">数据库表前缀:</div>
	<div class="x_col2"><input type="text" name="DB_PRE" value="<?php echo fun_get::get('DB_PRE','kj_');?>" class="pTxt1 pTxtL200"> <font style="color:#ff0000">*</font></div>
	<div class="x_col3 pBeta">同一个数据库运行多个系统时，用来区别表</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_title">网站配置信息</div>
	<div class="x_col1">管理员账号:</div>
	<div class="x_col2"><input type="text" name="WEB_ADMIN" value="<?php echo fun_get::get('WEB_ADMIN','admin');?>" class="pTxt1 pTxtL200"> <font style="color:#ff0000">*</font></div>
	<div class="x_col3 pBeta">初始管理员账号</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_col1">管理员密码:</div>
	<div class="x_col2"><input type="password" name="WEB_ADMIN_PWD" value="<?php echo fun_get::get('WEB_ADMIN_PWD');?>" class="pTxt1 pTxtL200"> <font style="color:#ff0000">*</font></div>
	<div class="x_col3 pBeta">请输入管理员密码</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_col1">二级目录地址:</div>
	<div class="x_col2"><input type="text" name="WEB_DIR" value="<?php echo fun_get::get('WEB_DIR' , $dirpath);?>" class="pTxt1 pTxtL200"></div>
	<div class="x_col3 pBeta">同一个域名下运行多个网站时用来区分各站点</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_col1">站点Cookie前缀:</div>
	<div class="x_col2"><input type="text" name="COOKIE_PRE" value="<?php echo fun_get::get('COOKIE_PRE','kj_');?>" class="pTxt1 pTxtL200"> <font style="color:#ff0000">*</font></div>
	<div class="x_col3 pBeta">同一个域名下运行多个网站时用来区分各站点</div>
	<div class="x_sp1">&nbsp;</div>
	<div class="x_title">官方注册信息</div>
	<div class="x_col1">账号:</div>
	<div class="x_col2"><input type="text" name="KLKKDJ_UNAME" value="<?php echo fun_get::get('KLKKDJ_UNAME');?>" class="pTxt1 pTxtL200"></div>
	...
	<div class="x_col2"><input type="password" name="KLKKDJ_PWD" value="<?php echo fun_get::get('KLKKDJ_PWD');?>" class="pTxt1 pTxtL200"></div>
...
<div class="div_box1" id="id_box2" style="display:none;height:220px;padding-top:80px">
	...
	<div class="div_action"><input type="button" name="btn1" value="访问网站" class="btn_1" onclick="location='<?php echo $dirpath;?>/';">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="btn1" value="后台管理" class="btn_1" onclick="location='<?php echo $dirpath;?>/master.php';"></div>
</div>
<script>
var thisjs = new function() {
	...
	this.pre = function() {
		...
		var url = kj.urlencode("<?php echo fun_get::url(array('app_act'=>'step2'));?>" , addparam)
		location = url;
	}
	...
	this.install_config = function() {
		...
		kj.ajax.post("<?php echo fun_get::url(array('app_act'=>'config'));?>" , obj_data , function(data) {
	...
	this.install_start = function() {
		...
		this.isstall = true;
		this.tables = [];
		kj.ajax.get("<?php echo fun_get::url(array('app_act'=>'install_gettable'));?>" , function(data) {
			var obj_data=kj.json(data);
			if(obj_data.isnull) {
				kj.alert("获取安装数据表时失败");
				thisjs.isstall = false;
	...
	this.install_table = function() {
		...
		if( thisjs.tables.length == 0 ) {
			...
			//插入初始账号
			kj.ajax.post("<?php echo fun_get::url(array('app_act'=>'initdata'));?>" , obj_data , function(data) {
				kj.progress.show1.close('install');
				kj.alert.show("安装完成");
				var arr = kj.obj("#id_left li");
		...
		kj.ajax.get("<?php echo fun_get::url(array('app_act'=>'install_table'));?>&tablename=" + tablename , function(data){
			var obj_data=kj.json(data);
			if(obj_data.isnull) {
	...
	this.install_row = function(tablename , row) {
		...
		kj.ajax.get("<?php echo fun_get::url(array('app_act'=>'install_row'));?>&tablename=" + tablename + "&page=" + row , function(data){
			var obj_data=kj.json(data);
			if(obj_data.isnull) {
				kj.alert("安装表[" + tablename + "]失败");
				thisjs.isstall = false;
