...
<div class="pMenu" id="id_pMenu">
	...
	<li class="fdpic" onclick="master_open({url:'common.php?app=config&app_module=user&dir=<?php echo $app_dir;?>&filename=other&key=other.sms',title:'设置字段',w:400});">&nbsp;</li>
</div>
<div class="btnMenuDiv" id="id_btnMenuDiv"<?php if($arr_list['issearch']==0){?> style="display:none"<?php }?>>
<li>发送时间：<input type="text" name="s_time1" value="<?php echo fun_get::get('s_time1');?>" class='pTxtDate' onfocus="new Calendar().show(this);"> 到 <input type="text" name="s_time2" value="<?php echo fun_get::get('s_time2');?>" class='pTxtDate' onfocus="new Calendar().show(this);"></li>
...
<li>是否回复：<select name="s_isre">
<option value="">不限</option>
<option value="1"<?php if('1'==fun_get::get("s_isre")){?> selected<?php }?>>已回复</option>
<option value="2"<?php if('2'==fun_get::get("s_isre")){?> selected<?php }?>>未回复</option>
</select></li>
...
<div class="pTableBox" id="id_table_box">
	...
	<div class="pTableList" id="id_table_list">
		<div class='pTable' id="id_table">
			...
			<div class='pTabRow'>
				<li><input type='checkbox' name='selid[]' value="<?php echo $item['sms_id'];?>"></li>
...
admin.table.list1.save_resize = function() {
	...
	kj.ajax.get("common.php?app=config&app_module=user&dir=<?php echo $app_dir;?>&app_act=save_resize&filename=other&key=other.sms&index=" + this.fieldsindex + "&w=" + lng_w , function(data){
		//alert(data);
	});
}
admin.table.list1.sort = function(key) {
	kj.ajax.get("common.php?app=config&app_module=user&filename=other&dir=<?php echo $app_dir;?>&app_act=sort&key=other.sms&sortby=" + key , function(data){
