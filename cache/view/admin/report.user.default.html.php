...
<div class="pMenu" id="id_pMenu">
	<li<?php if(fun_get::get("channel")==''){?> class="sel"<?php }?> onclick="admin.refresh_url(['channel=']);">注册用户</li>
	<li<?php if(fun_get::get("channel")=='login'){?> class="sel"<?php }?> onclick="admin.refresh_url(['channel=login']);">登录用户</li>
	<li<?php if(fun_get::get("channel")=='login_back'){?> class="sel"<?php }?> onclick="admin.refresh_url(['channel=login_back']);">回头客</li>
</div>
...
$(function () {
    ...
    $(document).ready(function() {
        chart = new Highcharts.Chart({
            ...
            yAxis: {
                title: {
                    text: '<?php if(fun_get::get("channel")=="login"){?>登录数<?php } else if(fun_get::get("channel")=="login_back") { ?>回头客<?php } else { ?>新用户<?php }?>'
                }
            },
            tooltip: {
                enabled: false,
            ...
            series: [{
                name: '<?php if(fun_get::get("channel")=="login"){?>登录数<?php } else if(fun_get::get("channel")=="login_back") { ?>回头客<?php } else { ?>新用户<?php }?>',
